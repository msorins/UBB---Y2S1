...
public class FXUIController {
    ...
    @FXML
    private TableView lockTable;

    @FXML
    private TableColumn<LockTableDataFX, Integer> lockTableIndex;

    @FXML
    private TableColumn<LockTableDataFX, Integer> lockTableBy;
    ...
    public void setOutHeapTable() {
        ...
        heapTableValue.setCellValueFactory( x -> x.getValue().getValue().asObject() );
        heapTableAddress.setCellValueFactory(x -> x.getValue().getAddress().asObject() );
    ...
    @FXML
    public void setOutLockTable() {
        ObservableList<LockTableDataFX> data = FXCollections.observableArrayList();
        HashMap<Integer, Integer> lockTableToy = null;
        try {
            lockTableToy = this.toyController.getPrograms().getAt(0).getLockTable().getData();
        } catch (AdtExceptions adtExceptions) {
            adtExceptions.printStackTrace();
        }

        lockTableIndex.setCellValueFactory( x -> x.getValue().getLockindex().asObject() );
        lockTableBy.setCellValueFactory( y -> y.getValue().getLockby().asObject() );


        for(Integer key : lockTableToy.keySet()) {
            Integer value =  lockTableToy.get(key);
            data.add(new LockTableDataFX(key, value));
        }

        if( lockTableToy != null && heapTable != null)
